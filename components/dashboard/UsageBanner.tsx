...
export default function UsageBanner() {
    ...
    const configs: Record<string, Config> = {
        warning: {
            ...
            icon: <Info className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />,
        ...
        critical: {
            ...
            icon: <AlertTriangle className="w-4 h-4 text-orange-600 shrink-0 mt-0.5" />,
        ...
        <div className={`${config.bg} border-b ${config.border} px-6 py-3`}>
            <div className="flex items-start gap-3 max-w-6xl mx-auto">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-600 mt-0.5 leading-relaxed">{config.text}</p>
                    {config.showUpgrade && (
                        <div className="flex items-center gap-3 mt-2">
                            ...
                            <span className="text-xs text-slate-400">or wait for the {resetFormatted} reset</span>
                        </div>
                    )}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                    {/* Usage badge */}
                    <div className="flex items-center gap-1 bg-white/70 rounded-lg px-2 py-1 border border-white/50">
                        <TrendingUp className="w-3 h-3 text-slate-400" />
                        <span className="text-[10px] font-bold text-slate-600">{used}/{cap}</span>
                    </div>
